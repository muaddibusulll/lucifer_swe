...
"""def pullrequest_inference(head_branch, base_branch, issue, diff):
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[{"role": "system", "content": pullrequestprompt(head_branch, base_branch, issue, diff)}],
        max_tokens=1500,
        temperature=0.0,
        n=1,
        stop=None
...
def main():
    # Input file paths
    codebase_file = "/Users/mohdtahaabbas/swe/codebasefile.txt"
    ...
    modified_codebase_file = f'codebase_modified_{current_time}.txt'
    ...
    diff_file = f'differencefile_{current_time}.txt'
